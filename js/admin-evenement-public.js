...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.adminPlanning = (function () {
    ...
    // Le calendrier existe maintenant (créé dans admin-planning.html)
    ...
    function refreshTable() {
      ...
      state.planningCreneaux.forEach((c) => {
        ...
        tr.innerHTML = `
          ...
          <td><span class="badge-dispo">${
            state.planningDisponibilites.filter(d => d.planning_creneau === c.id).length
          } dispo</span></td>
          <td>${c.etat || "—"}</td>
          <td><button class="btn-admin-secondary" style="font-size:0.75rem;">Voir</button></td>
        `;
